...
public class StreamEnhancer<T extends BaseEntity> implements Spliterator<T> {
    static ExecutorService executor = new ThreadPoolExecutor(3, 10, 10L, TimeUnit.MINUTES, new ArrayBlockingQueue<>(10), r -> {
        Thread thread = new Thread(r);
        thread.setName("StreamEnhancer-" + thread.getId());
        return thread;
