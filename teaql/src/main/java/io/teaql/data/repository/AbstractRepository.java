...
public abstract class AbstractRepository<T extends Entity> implements Repository<T> {
    ...
    private boolean parallelEnhancer = true;

    private static ExecutorService enhancerPool = new ThreadPoolExecutor(3, 10, 10L, TimeUnit.MINUTES, new ArrayBlockingQueue<>(10), r -> {
        Thread thread = new Thread(r);
        thread.setName("TEAQL-Repository-enhancer-" + thread.getId());
        return thread;
    ...
    public void enhanceRelations(
        ...
        for (Map.Entry<String, SearchRequest> entry : enhanceProperties.entrySet()) {
            String p = entry.getKey();
            SearchRequest r = entry.getValue();
            ...
            if (shouldHandle((Relation) property)) {
                if (!isParallelEnhancer()) {
                    enhanceParent(userContext, dataSet, (Relation) property, r);
                }
                else {
                    Future<?> done = enhancerPool.submit(() -> enhanceParent(userContext, dataSet, (Relation) property, r));
                    futures.add(done);
                }
            }
            else {
                if (!isParallelEnhancer()) {
                    ...
                    Future<?> done = enhancerPool.submit(() -> collectChildren(userContext, dataSet, (Relation) property, r));
                    futures.add(done);
                }
            }
        }
    ...
    public boolean isParallelEnhancer() {
        return parallelEnhancer;
    }

    public void setParallelEnhancer(boolean parallelEnhancer) {
        this.parallelEnhancer = parallelEnhancer;
    }
}
