...
public class SQLLogger {
    ...
    public static String finalSQLToExecute(String sql, Map<String, ?> paramMap) {

        StringBuilder finalSQLBuilder = new StringBuilder();
        String finalSQL = NamedParameterUtils.substituteNamedParameters(sql, null);
        Object[] parameters = NamedParameterUtils.buildValueArray(sql, paramMap);
        Counter counter = new Counter();
        char[] sqlChars = finalSQL.toCharArray();
        int index = 0;
        for (char ch : sqlChars) {
            counter.onChar(ch);
            if (ch == '?' && counter.outOfQuote()) {
                finalSQLBuilder.append(wrapValueInSQL(parameters[index]));
                index++;
                continue;
            }
            finalSQLBuilder.append(ch);
        }
